...
function App() {
  ...
  const fetchStkHandler = async (event) => {
    ...
    try {
      let response;
      if (period1 === '' && period2 === '') {
        response = await fetch(`http://127.0.0.1:5000/StkPrice/${stk}`);
      }
      else {
        ...
        response = await fetch(`http://127.0.0.1:5000/StkPrice/${stk}/date?period1=${period1.replaceAll('-', '')}&period2=${period2.replaceAll('-', '')}`);
      }
    ...
    <React.Fragment>
      <section>
        <form onSubmit={fetchStkHandler}>
          <label htmlFor='stk_name'>Stock Name/Ticker:</label>
